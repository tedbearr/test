  ...
  return (
    <>
      <div
        className={` ${
          status ? "block" : "hidden"
        } fixed max-sm:h-full max-sm:p-10 p-40 left-0 items-center top-0 w-full h-full overflow-auto z-50 bg-black bg-opacity-40`}
      >
        <div className="flex space-y-8 align-middle justify-center flex-col w-full h-auto bg-white rounded-2xl p-5">
          ...
          <div className="flex max-sm:flex-wrap max-sm:w-full max-sm:flex-grow-0 w-full h-full flex-col flex-wrap overflow-auto">
            {Children.map(
              children,
              (child) =>
